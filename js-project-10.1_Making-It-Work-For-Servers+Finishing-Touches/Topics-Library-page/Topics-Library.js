import {contentData} from './Topics-Library-Data.js';
// this one is 22 pages long import {contentData} from './Content-List_Data22.js';
...
export const search=document.getElementById('Search-Text');
...
//when pressed takes you to the Home Page
...
function loadContent(){
 ...
 if(''===sessionStorage.getItem('savedSearch')){
...
if(event.target.parentElement.classList.contains('Content-Container')){
  sessionStorage.setItem('Topic', event.target.parentElement.children[1].innerText);
};


if(event.target.classList.contains('Content-Container')){
  sessionStorage.setItem('Topic', event.target.children[1].innerText);
};
